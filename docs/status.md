...
The market feed displays readable cards and numerical price changes, filtering formatting-only events while preserving stored history. Benchmark APIs use only cited claims extracted during manually approved Z.ai research. Catalog adapters no longer fill benchmark tables. AI-extracted results remain unverified, separate score units, retain report links/dates and cannot influence rankings or recommendations.

Private manual AI research uses only Z.ai search plus cited summaries, explicit model approval, local usage limits and no retries or fallback. Complete cited benchmark claims are written only to an isolated unverified table; notes cannot change prices, identities or source evidence. Live execution requires `ZAI_API_KEY` and general API access; only mocked provider flows have been tested. This is the application's only provider API key. Harness and evaluation screens are removed for now; their authenticated API operations are paused by default and existing history remains intact.
