...
Manual research is a separate optional channel. OpenAI uses [Responses web search](https://developers.openai.com/api/docs/guides/tools-web-search) with a search-domain allowlist. Z.ai uses its documented [web-search API](https://docs.z.ai/api-reference/tools/web-search), filters returned excerpts locally to approved domains, then makes one [chat-completion request](https://docs.z.ai/api-reference/llm/chat-completion) for a cited summary. The Z.ai general API is separate from its Coding Plan endpoint ([authentication and endpoint guidance](https://docs.z.ai/api-reference/introduction)). Each provider uses its own key and requires approval. Clickable citations support review; they do not verify model-generated claims or authorize ingestion. Results remain private notes and are never fed into deterministic source adapters automatically.
