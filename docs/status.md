...
Private manual AI research uses only Z.ai search plus cited summaries, explicit model approval, local usage limits and no retries or fallback. Notes remain unverified and cannot update the catalog. Live execution requires `ZAI_API_KEY` and general API access; only mocked provider flows have been tested. This is the application's only provider API key. Harness and evaluation screens are removed for now; their authenticated API operations are paused by default and existing history remains intact.
