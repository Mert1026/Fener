...
Real models.dev and LiteLLM ingestion; raw snapshots and fetch receipts; source-scoped model aliases; explicit canonical identities; provider/access/endpoint separation; append-only facts, prices, conflicts and events; per-field confirmation freshness; durable manual sync requests; scheduled polling; deployment identity correction with review evidence and audit history. OpenRouter and LLM Stats integrations are retired: prior evidence remains, while new calls and queue entries are impossible.
...
The market feed displays readable cards and numerical price changes, filtering formatting-only events while preserving stored history. The Benchmarks page queues a durable catalog-wide AI research refresh from one button and shows progress across all resolved models. Catalog adapters and ordinary manual notes do not fill benchmark tables. AI-researched results remain unverified, separate score units, retain report links/dates and cannot influence rankings or recommendations.

Private manual AI research uses only Z.ai search plus cited summaries, explicit model approval, local usage limits and no retries or fallback. The separate catalog-wide benchmark refresh writes complete cited claims only to an isolated unverified table; manual notes cannot change prices, identities, benchmarks or source evidence. Live execution requires `ZAI_API_KEY` and general API access; only mocked provider flows have been tested. This is the application's only provider API key. Harness and evaluation screens are removed for now; their authenticated API operations are paused by default and existing history remains intact.

## Still gated or intentionally unfinished

| Area                 | Current limit / next step                                                                                                                                                                                                |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Quality intelligence | AI-extracted benchmark claims are unverified and deliberately excluded from the quality frontier. A review/approval workflow is required before any claim can influence ranking.                                         |
| Endpoint performance | No measured latency/throughput series is available from the active public sources. Historical uptime evidence is retained when present.                                                                                  |
