...
- `GET /benchmarks` (optional `group_id`) and `/benchmarks/groups`: latest AI-researched observations grouped by exact benchmark name, version, evaluator and metric. Report URL/date, cited page title and methodology gaps accompany results.
...
`GET /benchmark-refresh` returns the latest private catalog-wide refresh status. `POST /benchmark-refresh` requires `request_id` and `acknowledge_cost: true`; it snapshots every resolved catalog model into a durable queue and returns 202 without waiting for provider calls. The worker processes one model at a time with at most one Artificial Analysis domain-filtered search and one 2,000-token summary, never retries failed or uncertain items, and reports model/result/failure progress. Only model-level Artificial Analysis Intelligence Index claims are accepted; Coding Agent Index claims are rejected. Complete claims enter `research_benchmarks` only when the citation is valid and the returned model name exactly matches the queued catalog model. `GET /benchmarks`, model-detail benchmark lists and `GET /benchmarks/groups` expose only those AI-researched rows. They are always `ai_extracted_unverified`; legacy catalog benchmark rows are hidden, and normalized benchmark analytics exclude every unreviewed AI extraction.
