...
Complete benchmark claims in a successful research response are extracted into `research_benchmarks` only when the cited evidence ID is valid and the model name has one exact catalog match. `GET /benchmarks`, model-detail benchmark lists and `GET /benchmarks/groups` expose only those Z.ai-extracted rows. They are always `ai_extracted_unverified`; legacy catalog benchmark rows are hidden, and normalized benchmark analytics exclude every unreviewed AI extraction.
