...
Benchmark updates use a separate explicitly triggered durable queue across all resolved models. Z.ai search is domain-filtered to Artificial Analysis and asks only for the model-level Artificial Analysis Intelligence Index. Coding Agent Index results are rejected because they include agent harness, tools and execution settings. A result must cite an Artificial Analysis page and supply the exact benchmark name, index version, evaluator, metric, score direction and exact catalog model name. The cited page title is retained so a tested reasoning-effort or model variant remains visible. Cohorts are grouped by exact benchmark name, version, evaluator and metric; missing Artificial Analysis coverage remains `no_evidence` rather than receiving an inferred score. No other catalog facts are changed.
